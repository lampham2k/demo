<?php 
session_start();
$name_receiver = $_POST['name_receiver'];
$phone_receiver = $_POST['phone_receiver'];
$address_receiver = $_POST['address_receiver'];

$id = $_SESSION['id'];
require 'admin/connect.php';
// $sql ="
// select max(id) from orders
// where id = $_SESSION['id']
// "; khi thực hiện câu lệnh này sẽ lấy ra thằng có id cao nhất trong bảng orders


$cart = $_SESSION['carts'];

$total_price = 0;
foreach($cart as $each){
    $total_price += $each['quantity'] * $each['price'];
}
$customer_id = $_SESSION['id'];
$status = 0; // mới đặt 
$created_at = date('Y-m-d');

$sql =" insert into orders(customer_id,name_receiver,phone_receiver,address_receiver,status,created_at,total_price)
        values('$customer_id','$name_receiver','$phone_receiver','$address_receiver','$status','$created_at','$total_price')
";
mysqli_query($connect , $sql);

$sql = "select max(id) from orders where customer_id = '$customer_id' ";
$result = mysqli_query($connect , $sql);
$order_id = mysqli_fetch_array($result)['max(id)'];
// order_id bằng mảng tại vị trí đầu tiên với value là maxid

foreach($cart as $product_id => $each){
   // duyệt tất cả mảng session cart với key là product và value là each
    $quantity = $each['quantity'];
    $sql = "insert into order_product(order_id,product_id,quantity)
    values('$order_id','$product_id','$quantity')";
    // die($sql);
    mysqli_query($connect , $sql);
}
mysqli_close($connect);
unset($_SESSION['carts']);
// nếu không unset mỗi lần tải lại trang sẽ thêm 1 hóa đơn nữa.

header('location:index.php?success=Order Success');


